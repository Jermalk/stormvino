...
> At 290+ lines, extract the largest section to `dev/CLAUDE-ref-N.md` and replace with a one-line pointer.
> Never load a `dev/CLAUDE-ref` file unless the user explicitly asks about its topic.
...
- `dev/PROGRESS.md` missing → create with empty NOW section, continue.
- `dev/SCRATCHPAD.md` missing → create it empty, continue.
- `dev/SESSION.md` missing → create empty, continue.
- `dev/SESSION.md` **non-empty** → **BROKEN SESSION DETECTED.** Read it aloud to user, ask "Continue from this state?" before proceeding.

**Normal re-entry — in this order:**
1. Read `dev/PROGRESS.md` — **NOW section only** (skip history).
2. Read `dev/SCRATCHPAD.md` — summarise in one paragraph. Write back as `## Carried over:` (first entry), then clear the rest.
3. Read only files named in dev/PROGRESS.md "Next action". If "Next action" is empty or absent → stop and ask.
...
→ Full gap analysis, implementation order, and Qwen format spec in `dev/CLAUDE-ref.md § Tool-Call Gap`.
...
|---|---|---|
| Session start | `dev/PROGRESS.md` NOW, `dev/SCRATCHPAD.md` | Everything else until needed |
...
|---|---|---|
| CLAUDE.md file budget | 290 lines (hard cap 320) | Extract largest section to `dev/CLAUDE-ref-N.md` |
...
## dev/PROGRESS.md — NOW section format
...
File `dev/PROGRESS.md` has two parts: history (append-only, skip on re-entry) and NOW (overwritten each session, always last). **During session-wrap: copy current NOW into History first, then overwrite NOW.** Never skip — it is the only audit trail.

---

## dev/DECISIONS.md — entry format
...
Read `dev/DECISIONS.md` only when the user explicitly asks about a past decision.

---

## dev/SCRATCHPAD.md discipline
...
- You have made a decision not yet in `dev/DECISIONS.md`.

Format: bullet points, max 5 lines per topic, no prose. Cleared at start of every session (carry-over paragraph replaces it).

---

## dev/SESSION.md — broken-session recovery

Live crash snapshot at `dev/SESSION.md`. Overwritten on every commit during a session; cleared (emptied to zero bytes) by `#session-wrap`.
...
2. Copy current NOW block verbatim into `dev/PROGRESS.md` History (append as `### YYYY-MM-DD — Session N (<hash>)`), then overwrite NOW with updated fields including **Tests** result.
3. Append to `dev/DECISIONS.md` — one entry per architectural decision made this session.
4. Clear `dev/SCRATCHPAD.md`, write one-paragraph session summary.
5. Clear `dev/SESSION.md` (write empty file — signals clean close).
...
- **Typing — 2nd order:** See `dev/coding_standards_python.json` (TypedDict, TypeAlias, Protocol, TypeVar). Apply only when the stated `apply_when` condition is met — not by default.
- **Conventions for AI coding tools:** See `dev/CONVENTIONS.md` — update it whenever a module is added or ownership changes.
...
| `dev/coding_standards_python.json` | Python typing and clean-code standards. 1st-order rules are inlined above; load this file only for 2nd-order techniques (TypedDict, TypeAlias, Protocol, TypeVar). |
...
| `dev/autotest/YYYYMMdd_<hash>.md` | PND recovery artifacts — live test and debug session logs |
